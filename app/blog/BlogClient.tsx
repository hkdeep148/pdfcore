...
export default function BlogPage() {
  ...
  return (
    <div className="min-h-screen font-['Inter',sans-serif] text-slate-900 antialiased selection:bg-indigo-100 selection:text-indigo-700 overflow-x-hidden bg-gradient-to-br from-[#F8F9FB] via-[#F8F9FB] to-[#EEF0F8]">
      ...
      <section className="pt-12 md:pt-20 pb-12 md:pb-16">
        <div className="max-w-[900px] mx-auto px-5 md:px-8 text-center">
          ...
          >
            ...
            <h1 className="text-[32px] leading-[1.1] sm:text-[44px] md:text-[54px] md:leading-tight font-extrabold tracking-tight text-[#07122E] mb-4">
              Master PDFs with{' '}
            ...
            <p className="text-[15px] md:text-[17px] text-slate-600 font-medium max-w-2xl mx-auto leading-relaxed mb-8">
            ...
            <div className="max-w-2xl mx-auto">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-14 pr-5 py-4 rounded-xl border-2 border-slate-200 bg-white text-[15px] text-[#07122E] placeholder-slate-400 focus:border-indigo-500 focus:outline-none focus:ring-4 focus:ring-indigo-500/10 transition-all shadow-[0_4px_20px_-8px_rgba(15,23,42,0.1)]"
                />
                {searchQuery && (
                  <button
                    type="button"
                    onClick={() => setSearchQuery('')}
                    className="absolute right-4 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center hover:bg-slate-200 transition-colors"
                    aria-label="Clear search"
                  >
                    <svg viewBox="0 0 24 24" className="w-4 h-4 text-slate-600" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round">
      ...
      {/* ============ CATEGORY FILTERS ============ */}
      <section className="py-6 md:py-8 border-y border-slate-200/60 sticky top-[72px] z-30 backdrop-blur-lg bg-white/70">
            ...
            <button
              ...
              className={`flex-shrink-0 inline-flex items-center gap-2 px-4 py-2.5 rounded-full text-[13px] font-bold transition-all ${
                activeCategory === 'all'
                  ...
                  : 'bg-white border border-slate-200 text-slate-600 hover:border-indigo-200 hover:text-[#07122E]'
              }`}
            >
              <Sparkles size={14} />
              All Articles
      ...
      <section className="py-10 md:py-16">
        <div className="max-w-[1200px] mx-auto px-5 md:px-8">
          <div className="flex items-center justify-between mb-6 md:mb-8 flex-wrap gap-3">
            <div>
              <h2 className="text-[24px] md:text-[32px] font-extrabold text-[#07122E] tracking-tight">
                {searchQuery
                  ? `Search Results`
                  : activeCategory === 'all'
                  ? 'Latest Articles'
          ...
          {gridPosts.length === 0 ? (
            <div className="text-center py-16 bg-white rounded-2xl border border-slate-100">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-indigo-600 text-white text-[14px] font-semibold hover:bg-indigo-700 transition-colors"
              >
                Clear filters
              </button>
            </div>
        ...
        <div className="max-w-[900px] mx-auto px-5 md:px-8">
          <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-indigo-600 via-purple-600 to-purple-700 px-8 py-14 md:px-16 md:py-20 text-center shadow-[0_20px_60px_-15px_rgba(109,53,255,0.5)]">
            ...
            <div className="relative">
              ...
              <h2 className="text-[26px] md:text-[38px] font-extrabold text-white leading-tight tracking-tight mb-4">
...
function PostCard({ post, index = 0 }: PostCardProps) {
  ...
  return (
    ...
    >
      <Link
        href={`/blog/${post.slug}`}
        className="group block bg-white rounded-2xl border border-slate-100 hover:border-indigo-200 hover:shadow-[0_12px_28px_-8px_rgba(15,23,42,0.12)] transition-all overflow-hidden h-full flex flex-col"
      >
        ...
        <div className="p-5 md:p-6 flex-1 flex flex-col">
          <h3 className="text-[16px] md:text-[17px] font-extrabold text-[#07122E] leading-snug mb-2 group-hover:text-indigo-600 transition-colors line-clamp-2">
            {post.title}
          </h3>
          ...
          <div className="flex items-center gap-3 text-[11.5px] text-slate-500 pt-3 border-t border-slate-100">
...
function FeaturedPostCard({ post, large }: FeaturedPostCardProps) {
  ...
  return (
    <Link
      href={`/blog/${post.slug}`}
      className={`group block bg-white rounded-2xl border border-slate-100 hover:border-indigo-200 hover:shadow-[0_12px_28px_-8px_rgba(15,23,42,0.12)] transition-all overflow-hidden ${
        large ? 'h-full' : ''
      }`}
    >
      <div className={`relative overflow-hidden ${
        ...
        <div className="absolute top-4 right-4">
          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10.5px] font-extrabold uppercase tracking-wider bg-amber-500 text-white shadow-lg">
            <TrendingUp size={11} />
            Featured
          </span>
        </div>
      ...
      <div className={`p-5 md:p-6 ${large ? 'md:p-8' : ''}`}>
        <h3 className={`font-extrabold text-[#07122E] leading-tight mb-3 group-hover:text-indigo-600 transition-colors ${
...
function CompactPostCard({ post }: CompactPostCardProps) {
  ...
  return (
    <Link
      href={`/blog/${post.slug}`}
      className="group flex items-center gap-4 p-4 bg-white rounded-xl border border-slate-100 hover:border-indigo-200 hover:shadow-[0_4px_16px_-4px_rgba(15,23,42,0.1)] transition-all"
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <h4 className="text-[13.5px] font-extrabold text-[#07122E] leading-snug group-hover:text-indigo-600 transition-colors line-clamp-2 mb-1">
          {post.title}
        </h4>
      ...
      <ArrowRight size={16} className="text-slate-400 group-hover:text-indigo-600 group-hover:translate-x-1 transition-all flex-shrink-0" />
    </Link>
  );
}