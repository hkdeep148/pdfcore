...
export default function DesktopView() {
        ...
        <div className="grid grid-cols-2 gap-1 p-1 rounded-lg bg-[#F1F5F9]">
          <button
            ...
            className={`px-3 py-2 rounded-md text-[12px] font-bold transition-all ${
              mode === 'quality' ? 'bg-white text-[#4F46E5] shadow-sm' : 'text-[#6B7280] hover:text-[#111827]'
            }`}
          >
            Quality
          </button>
          <button
            ...
            disabled={processing}
            className={`px-3 py-2 rounded-md text-[12px] font-bold transition-all ${
              mode === 'size' ? 'bg-white text-[#4F46E5] shadow-sm' : 'text-[#6B7280] hover:text-[#111827]'
            }`}
          >
            Target Size
          </button>
              ...
              return (
                <button
                  ...
                  className={`relative p-3 rounded-lg border-2 transition-all ${
                    isSelected ? 'border-[#4F46E5] bg-[#EEF2FF]' : 'border-[#E8EDF5] bg-white hover:border-[#C7D2FE]'
                  } disabled:opacity-50`}
                >
                  {preset.recommended && (
                  ...
                  <div className={`text-[18px] mb-1 ${preset.recommended ? 'mt-1.5' : 'mt-0'}`}>
                    {preset.icon}
                  </div>
  ...
  const actionButton = files.length > 0 ? (
    !hasCompleted ? (
      ...
      >
        {processing ? (
          <><Loader2 size={16} className="animate-spin" strokeWidth={2.5} />Compressing...</>
        ) : (
          <><Zap size={16} strokeWidth={2.5} />Compress {files.length} {files.length === 1 ? 'Image' : 'Images'}</>
        )}
      </button>
    ) : (
      <div className="space-y-2">
        ...
        >
          {zipping ? (
            <><Loader2 size={16} className="animate-spin" strokeWidth={2.5} />Creating ZIP...</>
          ) : completedCount > 1 ? (
            <><Package size={16} strokeWidth={2.5} />Download ZIP ({completedCount})</>
          ) : (
        ...
        >
          {processing ? (
            <><Loader2 size={14} className="animate-spin" strokeWidth={2.5} />Re-compressing...</>
          ) : (
            <><RotateCcw size={14} strokeWidth={2.5} />Re-compress</>
          )}
        </button>
      </div>
    )
    ...
    >
      ...
      {files.length === 0 ? (
        ...
        <div
          ...
          className={`flex-1 rounded-2xl border-2 border-dashed transition-all flex items-center justify-center ${
            isDragging
              ? 'border-[#4F46E5] bg-[#EEF2FF]'
              : 'border-[#E8EDF5] bg-white hover:border-[#C7D2FE]'
          }`}
        >
          <label
            ...
            className="flex flex-col items-center justify-center py-16 px-8 cursor-pointer w-full max-w-md text-center"
          >
            <input
              type="file"
              accept="image/jpeg,image/jpg,image/png,image/webp"
...
// ============ FILE CARD (Horizontal, Clean, Minimal) ============
      ...
      <div className="flex-1 min-w-0 flex flex-col justify-between py-0.5">
        <div>
          ...
          <div className="flex items-start gap-2 mb-1.5">
            <p className="text-[14px] font-bold text-[#111827] truncate flex-1" title={file.original.name}>
              {file.original.name}
            </p>
            ...
            {formatChanged ? (
              <div className="flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-slate-100 text-[9px] font-bold uppercase tracking-wider shrink-0">
                ...
                <svg viewBox="0 0 24 24" className="w-2 h-2 text-slate-400" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
          ...
          <div className="flex items-center gap-2 text-[12px] mb-1.5">
            {file.status === 'done' && file.compressedSize ? (
              <>
                ...
                <svg viewBox="0 0 24 24" className="w-3 h-3 text-[#9CA3AF]" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                  <polyline points="9 18 15 12 9 6" />
                </svg>
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {file.status === 'done' && file.reduction !== undefined && file.compressedSize !== undefined && file.originalSize > file.compressedSize && (
              <>
      ...
      <div className="relative" ref={dropdownRef}>
        <button
          ...
          className={`w-full flex items-center gap-2 px-3 py-2.5 rounded-lg border-2 transition-all ${
            isOpen ? 'border-[#4F46E5] bg-[#EEF2FF]' : 'border-[#E8EDF5] bg-white hover:border-[#C7D2FE]'
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-2 px-3 py-2.5 transition-colors ${
                    isSelected ? 'bg-[#EEF2FF] text-[#4F46E5]' : 'text-[#111827] hover:bg-[#F8FAFF]'
                  }`}
                >
                  ...
                  {isSelected && (
                    <svg viewBox="0 0 24 24" className="w-3.5 h-3.5 text-[#4F46E5]" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
                      <polyline points="20 6 9 17 4 12" />
                    </svg>
                  )}
                </button>
