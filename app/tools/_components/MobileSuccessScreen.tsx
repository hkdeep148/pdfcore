...
interface MobileSuccessScreenProps {
  ...
  onStartOver: () => void;
  onBack: () => void;
  /** Optional status badge shown on file card (e.g., "UNLOCKED", "COMPRESSED") */
  statusBadge?: {
    label: string;
    color?: 'green' | 'blue' | 'purple' | 'orange';
  };
  /** Custom icon for the file card (overrides default red PDF badge) */
  iconVariant?: 'pdf' | 'unlocked' | 'locked';
}
  ...
  return (
    <div className="flex flex-col h-full bg-[#F4F5F7]">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-8 min-h-0 overflow-y-auto">
        ...
        {/* File Info Card */}
<div className="w-full max-w-[360px] bg-white rounded-2xl border border-[#E5E7EB] p-4 mb-6 shadow-[0_2px_8px_rgba(0,0,0,0.04)]">
    ...
    <div className="flex-1 min-w-0">
      ...
      <div className="flex items-center gap-2 mt-1 flex-wrap">
        ...
        <div className="w-full max-w-[360px] space-y-2.5">
          ...
          <button
            onClick={onStartOver}
            className="w-full h-[48px] bg-transparent text-[#6B7280] text-[14px] font-semibold rounded-[14px] active:scale-[0.98] transition-transform flex items-center justify-center gap-2 hover:text-[#07122E] transition-colors"
          >
